...
args.learning_rateD = 4e-05
args.learning_rateW = 4e-05
...
optimizerW = torch.optim.Adam(Where_net.parameters(), lr=args.learning_rateW)
optimizerD = torch.optim.Adam(Dis_net.parameters(), lr=args.learning_rateD)
...
if args.trained:
    checkpoint = torch.load(args.model_dir + '/model_' + args.where_net + '_' + args.threat_item + '.h5', map_location=device)
...
for epoch in range(args.epochs):
    ...
    for local_batch in range(training_range):
        ...
        # zero gradient with Dis net
        Dis_net.zero_grad()
        # Discriminator with real tensors
        outReals = Dis_net(r_imgs).view(-1)
        # Format target labels
        label = torch.empty_like(outReals)
        label.data.fill_(1.0)
        target = label.to(device)
        d_real_loss = criterion1(outReals, target)
        # Calculate gradients for D in backward pass
        d_real_loss.backward()

        if local_batch % 2 == 0:
            # Discriminator with composite imgs
            outCompsD = Dis_net(bg_imgs).view(-1)
            # state as label again
            label.data.fill_(0.0)
            target = label.to(device)
            d_fake_losscom = criterion1(outCompsD, target)
            # Calculate gradients for D in backward pass
            d_fake_losscom.backward()  # retain_graph=True)
        else:
            # generate the detach compose image
            newfg_imgs = Where_net(fg_imgs, bg_imgs).detach()
            # FG BG composite
            FG_masks = torch.where(newfg_imgs < 0.6, 1, 0)
            com_imgs = newfg_imgs * FG_masks * args.FG_blending + bg_imgs * (1 - FG_masks * args.BG_blending)

            # Discriminator with composite imgs
            outCompsD = Dis_net(com_imgs.detach()).view(-1)
            # state as label again
            label.data.fill_(0.0)
            target = label.to(device)
            d_fake_losscom = criterion1(outCompsD, target)
        ...
        # count the loss of the Dis net and update Dis net
        errD = d_real_loss + d_fake_losscom  # + d_fake_lossbg
        optimizerD.step()
        ...
        if local_batch % 4 == 0:
            ...
            # generate compose image for training
            newfg_imgs = Where_net(fg_imgs, bg_imgs)
            # FG BG composite
            FG_masks = torch.where(newfg_imgs < 0.6, 1, 0)
            com_imgs = newfg_imgs * FG_masks * args.FG_blending + bg_imgs * (1 - FG_masks * args.BG_blending)
            ...
            Where_net.zero_grad()
            # into the dis net
            outCompsW = Dis_net(com_imgs).view(-1)
            label.data.fill_(1.0)
            target = label.to(device)
            g_fake_loss = criterion1(outCompsW, target)
            # Calculate gradients for D in backward pass
            g_fake_loss.backward()
            # count the loss of the STN net and update the STN net
            errG = g_fake_loss
            optimizerW.step()
        ...
        if epoch % 2 == 0 and local_batch == 35:
            # save the sample results
            # torchvision.utils.save_image(fg_imgs.cpu(), args.sample_dir + args.threat_item + '/FG_' + str(epoch) + args.where_net + "_" + str(local_batch) +".png")
            # torchvision.utils.save_image(newfg_imgs.cpu(), args.sample_dir + args.threat_item + '/nFG_' + str(epoch) + args.where_net + "_" + str(local_batch) + ".png")
            torchvision.utils.save_image(com_imgs.cpu(), args.sample_dir + args.threat_item + '/COM_' + str(epoch) + args.where_net + "_" + str(local_batch) + ".png")
    ...
    torch.save(state, args.model_dir + '/model_' + args.where_net + '_' + args.threat_item.split('/')[1] + '.h5')
...
plt.savefig(args.sample_dir + '/loss_' + args.where_net + ".png")